...
# using Parameters
# CC parameter struct
@with_kw struct CCParameters @deftype Float64
    Tmax
    T0
    ωmax
    ωc
...
torque_angvel_norm(cc::CC) = torque_angvel(-cc.ω, cc.cc_p.Tmax / cc.cc_p.T0, 1., cc.cc_p.ωmax, cc.cc_p.ωc, cc.cc_p.Amin, cc.cc_p.ω1, cc.cc_p.ωr)    
torque_angvel_norm(ω, p::CCParameters) = torque_angvel(ω, p.Tmax / p.T0, 1., p.ωmax, p.ωc, p.Amin, p.ω1, p.ωr)    
...
Plots.surface(p::CCParameters) = surface((p.θopt - 3p.θopt):0.01:(p.θopt + 3p.θopt), -p.ωmax:0.01:p.ωmax, (θ, ω) -> torque_norm(θ, ω, p), xaxis="angle", yaxis="angular velocity", zaxis="normalised torque")