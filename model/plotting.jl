function plot_model(sol, t)
    ...
    plot(legend=:none, grid=:off, xlims=(-.8, 5.0), ylims=(-.1, 5.7), axis=nothing, border=:none, lw=2, color=:black)
    plot!(ps[1:2:16], ps[2:2:16], color=:black)     # main skeleton
    plot!(ps[[11,17]], ps[[12,18]])     # HAT
    plot!(ps[[3,7]], ps[[4,8]])     # connect foot
