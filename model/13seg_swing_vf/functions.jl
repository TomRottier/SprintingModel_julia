...
function vocmx(u, p, t)
    @unpack footang, lff, lffo, lhat, lhato, lla, llao, lrf, lrff, lrffo, lrfo, lsh, lsho, lth, ltho, lua, luao, mff, mhat, mla, mrf, msh, mth, mtpb, mtpk, mua, u8, u9, u10, u11, u12, u13, u14 = p
...
function pocmx(u, p, t)
    ...
    return q1 + 0.5 * (2 * llao * mla * cos(ls - le - q3) + 2 * llao * mla * cos(rs - re - q3) + 2 * (lhato * mhat + 2 * lhat * mla + 2 * lhat * mua) * cos(q3) + (2 * lrf * mff + mrf * (2 * lrf - lrfo)) * cos(ra + rh - rk - q3) + 2 * (lth * mff + lth * mrf + lth * msh + mth * (lth - ltho)) * cos(rh - q3) + 2 * (lsh * mff + lsh * mhat + lsh * mrf + lsh * msh + lsho * msh + 2 * lsh * mla + 2 * lsh * mth + 2 * lsh * mua) * cos(q4 - q3 - q5) + 2 * (lff * mff + lff * mhat + lffo * mff + 2 * lff * mla + 2 * lff * mrf + 2 * lff * msh + 2 * lff * mth + 2 * lff * mua) * cos(q3 + q5 - q4 - q6 - q7) - 2 * (lua * mla + luao * mua) * cos(ls - q3) - 2 * (lua * mla + luao * mua) * cos(rs - q3) - lrffo * mrf * cos(footang + q3 + q5 - q4 - q6) - lrffo * mrf * cos(ra + rh - footang - rk - q3) - 2 * mff * (lff - lffo) * cos(ra + rh + rmtp - rk - q3) - 2 * (lsh * mff + lsh * mrf + msh * (lsh - lsho)) * cos(rh - rk - q3) - 2 * (lth * mff + lth * mhat + lth * mrf + lth * msh + lth * mth + ltho * mth + 2 * lth * mla + 2 * lth * mua) * cos(q3 - q4) - (lrfo * mrf + 2 * lrf * mff + 2 * lrf * mhat + 2 * lrf * mrf + 4 * lrf * mla + 4 * lrf * msh + 4 * lrf * mth + 4 * lrf * mua) * cos(q3 + q5 - q4 - q6)) / (mhat + 2 * mff + 2 * mla + 2 * mrf + 2 * msh + 2 * mth + 2 * mua)
end
function pocmy(u, p, t)
    ...
    return q2 + 0.5 * (2 * (lua * mla + luao * mua) * sin(ls - q3) + 2 * (lua * mla + luao * mua) * sin(rs - q3) + lrffo * mrf * sin(ra + rh - footang - rk - q3) + 2 * (lhato * mhat + 2 * lhat * mla + 2 * lhat * mua) * sin(q3) + 2 * mff * (lff - lffo) * sin(ra + rh + rmtp - rk - q3) + 2 * (lsh * mff + lsh * mrf + msh * (lsh - lsho)) * sin(rh - rk - q3) + 2 * (lff * mff + lff * mhat + lffo * mff + 2 * lff * mla + 2 * lff * mrf + 2 * lff * msh + 2 * lff * mth + 2 * lff * mua) * sin(q3 + q5 - q4 - q6 - q7) - 2 * llao * mla * sin(ls - le - q3) - 2 * llao * mla * sin(rs - re - q3) - lrffo * mrf * sin(footang + q3 + q5 - q4 - q6) - 2 * (lth * mff + lth * mrf + lth * msh + mth * (lth - ltho)) * sin(rh - q3) - (2 * lrf * mff + mrf * (2 * lrf - lrfo)) * sin(ra + rh - rk - q3) - 2 * (lth * mff + lth * mhat + lth * mrf + lth * msh + lth * mth + ltho * mth + 2 * lth * mla + 2 * lth * mua) * sin(q3 - q4) - 2 * (lsh * mff + lsh * mhat + lsh * mrf + lsh * msh + lsho * msh + 2 * lsh * mla + 2 * lsh * mth + 2 * lsh * mua) * sin(q4 - q3 - q5) - (lrfo * mrf + 2 * lrf * mff + 2 * lrf * mhat + 2 * lrf * mrf + 4 * lrf * mla + 4 * lrf * msh + 4 * lrf * mth + 4 * lrf * mua) * sin(q3 + q5 - q4 - q6)) / (mhat + 2 * mff + 2 * mla + 2 * mrf + 2 * msh + 2 * mth + 2 * mua)
end
