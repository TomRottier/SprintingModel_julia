% torque driven legs, angle driven arms 
% only simulate half stride
%
% tom rottier 2022
% ------------------------------------------------------------------------------
autoz on          
...
variables q{11}'		    % global position (x,y) and orientation of HAT, 4 joint angles each leg
variables u{15}'            % generalised speeds, u11-u15 to determine torques at upper body joints
variables lrx{2},lry{2}		% one spring under toe, one under mtp
variables kecm,pecm,te,hz,px,py
variables rstq,lstq,retq,letq
variables lh'',lk'',la'',lmtp'',rh'',rk'',ra'',rmtp''
...
constants lhe,lke,lae,lhf,lkf,laf,rhe,rke,rae,rhf,rkf,raf % parameters of torque generators
specified rs'',ls'',re'',le'' % specified joint angles
specified lhtq,lktq,latq,lmtq,rhtq,rktq,ratq,rmtq
%
%
% ------------------------------------------------------------------------------
% geometry
simprot(n,hat,3,q3)         % global orientation/trunk angle
simprot(hat,rth,3,2*pi - q8)
simprot(hat,lth,3,2*pi - q4)
simprot(rth,rsh,3,q9 - pi)
simprot(lth,lsh,3,q5 - pi)
simprot(rsh,rrf,3,pi - q10)
simprot(lsh,lrf,3,pi - q6)
simprot(rrf,rff,3,pi - q11)
...
w_rff_rrf> = -u11*n3>
w_lff_lrf> = -u7*n3>
w_rua_hat> = -rs'*n3> + u12*n3>
w_lua_hat> = -ls'*n3> + u13*n3>
w_rla_rua> = re'*n3> + u14*n3>
w_lla_lua> = le'*n3> + u15*n3>
w_rrff_rrf> = 0>
w_lrff_lrf> = 0>
...
rx1 = (-k1*dx1-k2*vop1x)*ry1
ry2 = -k7*pop2y-k8*vop2y*abs(pop2y)         % mtp
rx2 = (-k5*dx2-k6*vop2x)*ry2
rx = rx1+rx2
ry = ry1+ry2
%
...
constrain(auxiliary[u12,u13,u14,u15])
zee_not = [rstq,lstq,retq,letq]
zero = fr() + frstar()
kane(rstq,lstq,retq,letq)
%
% ------------------------------------------------------------------------------
% input/output
...
output t,q1,q2,q3,q4,q5,q6,q7,q8,q9,q10,q11,u1,u2,u3,u4,u5,u6,u7,u8,u9,u10,u11
output t,rx,ry,rx1,rx2,ry1,ry2
...
%
%--------------------------------------------------------------
code dynamics() model/13seg_tq/model.f, nosubs
